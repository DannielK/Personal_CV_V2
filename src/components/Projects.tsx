  ...
  return (
    <ol className="relative">
      ...
      {PROJECTS.map((project, index) => (
        <li
          key={index}
          className={`group relative mb-10 grid gap-4 rounded-md p-2 pb-1 backdrop-blur-sm transition-all duration-100 sm:grid-cols-8 sm:gap-8 md:gap-4 lg:p-4 ${
            hoveredIndex !== null && hoveredIndex !== index
              ? "opacity-50"
              : "opacity-100"
          } ${hoveredIndex === index ? "bg-slate-800/50 drop-shadow-lg backdrop-blur-md" : ""} `}
